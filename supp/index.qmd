---
title: "An empirical assessment of biface geochemistry and morphology as a function of raw material color"
author:
...
The physical characteristics of lithic raw materials are routinely advanced as a determining factor in the morphology of stone tools, where nodule size and mechanical flaking properties are generally assumed to account for morphological differences that occur between sites and assemblages. This study employs a quantitative analysis of raw material color to assign bifaces to distinct color groups, then asks whether the geochemistry and/or morphology of those bifaces can be said to differ. Color is often used as the determining characteristic in lithic raw material assignments, and new analytical tools allow for the objective and replicable analysis of raw material color. Two raw material color groups were identified for Gahagan bifaces from the Gahagan Mound and Mounds Plantation sites in the American Southeast. Findings demonstrate that Gahagan bifaces differ significantly in morphology, but not geochemistry, by raw material color group. Thus, this study yields support for the argument that lithic morphology differs between raw material color groups, supporting the interpretation that extralocal producers---and local Caddo users/knappers---conditioned Gahagan biface shape not upon nodule size or mechanical flaking properties, but on the basis of raw material color. Given these findings, paired with the absence of local production evidence, it is posited that Gahagan bifaces from Caddo mortuary contexts at Gahagan Mound and Mounds Plantation were acquired through trade and/or exchange with producers that operated under similar constraints, but who manufactured Gahagan bifaces to be morphologically distinct when using raw material associated with specific color ranges.
...
The Gahagan bifaces were scanned at 600 dpi using an HP ScanJet G4050. Images were subsequently transferred to a transparent background in Photoshop in preparation for analysis using the `colordistance` package in R [@R;@RN11200]. A histogram binning method was used to group similar colors, and pairwise distances between histograms were computed using earth mover’s distance, yielding two distinct raw material color groups [@RN11201].
...
Bifaces were coded as ColorGroup A and ColorGroup B, then the color distance matrix was exported, joined with the qualitative data, then analyzed using a permutational multivariate analysis of variance (perMANOVA) in the `vegan` package [@R;@vegan], where the color groups were found to differ significantly (_permutations = 10,000; Rsq = 0.48746; **Pr(>F) = 9.999e-05**_). Ordination was performed using non-metric multidimensional scaling (NMDS) to illustrate the color ranges for Gahagan bifaces from ColorGroup A and ColorGroup B.

```{r colorspace, out.width = "100%", dpi = 300, echo=FALSE, warning=FALSE, fig.cap="NMDS summarizing color variation in Gahagan bifaces, where ColorGroup A is depicted in gray, and the ColorGroup B is depicted in orange with each point plotted in color space to visualize color variation."}
...
The PCA illustrates 59.32 percent of the variation in the sample among PC1 (42.81 percent) and PC2 (16.51 percent). The perMANOVA demonstrated that elemental values differ significantly by raw material color groups (_permutations = 10,000; **Pr(>F) = 0.06379**_).
