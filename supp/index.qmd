...
Archaeologists regularly assume that the physical properties of lithic raw materials are a determining factor in the morphology of stone tools; although, some have argued that this assumption warrants regular testing. Raw material nodule size and mechanical flaking properties are routinely advanced to account for morphological differences that occur between sites and assemblages. Lithic raw material assignments often use color as the determining characteristic, and new analytical tools allow for an objective analysis of raw material color. Two raw material color groups---both assumed to be Edwards Formation cherts---were identified for Gahagan bifaces from the Gahagan Mound and Mounds Plantation sites in the American Southeast. This study asks whether the geochemistry and morphology of Gahagan bifaces differs as a function of raw material color group. Findings demonstrate that Gahagan bifaces differ significantly in geochemistry, but not in morphology, by raw material color group. Thus, this study yields support for the argument that lithic raw material properties differ between Edwards Formation color groups, while also providing support for the assumption that biface morphology is a function of the general raw material source. Given these findings, paired with the absence of local production evidence, it is posited that Gahagan bifaces from Caddo mortuary contexts at Gahagan Mound and Mounds Plantation were acquired through trade and/or exchange with producers who operated under a similar mental template, and who manufactured the Gahagan bifaces from Edwards Formation sources distinct in both color and geochemistry.
...
To assess whether `shape` and `size` differs by `ColorGroup`, Procrustes ANOVAs [@RN1749] were run that enlist effect-sizes (zscores) computed as standard deviates of the generated sampling distributions [@RN1756]. A residual randomization permutation procedure (RRPP; n = 10,000 permutations) was used for all Procrustes ANOVAs [@RN1655;@RN11775], which has higher statistical power and a greater ability to identify patterns in the data should they be present [@RN1719]. An analysis of morphological disparity was used to identify potential differences in shape and size diversity between sites [@RN11107;@RN7041;@RN5694].
  ...
  -   Gahagan biface shape is not correlated with raw material color groups
  -   Gahagan biface size is not correlated with raw material color groups
