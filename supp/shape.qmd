...
```{r m-disparity}
# morphological disparity by site
morphol.disparity(fit.shape, 
                  groups = qdata$sitename, 
                  data = gdf, 
                  print.progress = FALSE, 
                  iter = 9999)
...
# elemental data
elemental <- qdata %>%
  select(c(Al.K12:Ba.L1, Ni.K12:Zn.K12))
...
#                            group = qdata$sitename)

#names(new.coords)

## plot shape means
#mean <- lapply(new.coords, mshape)
#plot(mean$`Gahagan Mound`)
#plot(mean$`Mounds Plantation`)

# comparison plots
#plotRefToTarget(mean$`Gahagan Mound`,
#                mean$`Mounds Plantation`, 
#                method = "points",
#                mag = 1)
