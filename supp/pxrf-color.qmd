...
```{r pca}
#pca
pch.gps <- c(15,17)[as.factor(raw$ColorGroup)]
ColorGroup <- raw$ColorGroup

## pca plot using correlation matrix
pca <- autoplot(prcomp(raw[,c(5:12)], scale. = TRUE),
                data = raw,
                asp = 1,
                shape = pch.gps,
                colour = "ColorGroup",
...
```{r permanova}
# perMANOVA
gahagan.matrix <- as.matrix(raw[,c(5:12)]) # response vars
